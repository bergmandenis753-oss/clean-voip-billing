...
def init_db() -> None:
    ...
    conn.executescript(
        ...
        -- Терминаторы (поставщики, кто даёт нам роут). Закупка.
        -- На префикс активен ровно один терминатор.
        CREATE TABLE IF NOT EXISTS termination_groups (
            id               INTEGER PRIMARY KEY AUTOINCREMENT,
            name             TEXT    NOT NULL UNIQUE, -- account/gateway name, e.g. 'Provider A'
        ...
        CREATE TABLE IF NOT EXISTS terminators (
            ...
            ips              TEXT    NOT NULL DEFAULT '',  -- IP поставщика через запятую
            destination_name TEXT    NOT NULL,       -- 'Australia'
            prefix           TEXT    NOT NULL,       -- '61'
            gateway_name     TEXT    NOT NULL,       -- имя sofia-gateway, напр. 'provider_a'
