def apply(ai_diag):
    ...
    def inject_dashboard(html):
        ...
        guard = """
const __voipOriginalFetch = window.fetch.bind(window);
window.fetch = (input, init = {}) => {
  ...
  if (url.includes('/api/dashboard-data')) {
    ...
    return __voipOriginalFetch(input, nextInit).finally(() => clearTimeout(timeout));
  }
  return __voipOriginalFetch(input, {...init, headers, credentials:'same-origin'});
};
"""
        if "__voipOriginalFetch" not in html:
            html = html.replace(
