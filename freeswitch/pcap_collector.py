#!/usr/bin/env python3
"""Passive SIP packet collector for VoIP billing diagnostics.
...
DEFAULT_LOCAL_IPS = "127.0.0.1"

API = os.getenv("BILLING_API", DEFAULT_API).rstrip("/")
KEY_FILE = os.getenv("BILLING_API_KEY_FILE", DEFAULT_KEY_FILE)
IFACE = os.getenv("VOIP_PCAP_IFACE", "any")
LOCAL_IPS = {
    token.strip()
    for token in os.getenv("VOIP_LOCAL_IPS", DEFAULT_LOCAL_IPS).replace(";", ",").split(",")
    if token.strip()
}
FILTER = os.getenv("VOIP_PCAP_FILTER", "udp port 5060 or udp port 5080")
BATCH_SIZE = int(os.getenv("VOIP_PCAP_BATCH_SIZE", "12"))
FLUSH_INTERVAL = float(os.getenv("VOIP_PCAP_FLUSH_INTERVAL", "1.5"))
MAX_SUMMARY = int(os.getenv("VOIP_PCAP_MAX_SUMMARY", "5000"))
